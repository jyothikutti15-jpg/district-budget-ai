    ...
    const scenario = await withTransaction(districtId, async (client) => {
      const result = await client.query<BudgetScenario>(
        `INSERT INTO budget_scenarios (
         ...
         RETURNING *`,
        [
          districtId, '00000000-0000-0000-0000-000000000002', fiscalYear, name, description || null,
...
- Projected savings: $${(Number(scenario.projected_savings || 0) / 100).toLocaleString()}
- Remaining deficit after cuts: $${Math.max(0, Number(scenario.projected_deficit || 0) / 100).toLocaleString()}

PROJECTED STUDENT IMPACT:
- Math proficiency: ${Number(scenario.math_proficiency_delta) > 0 ? '+' : ''}${Number(scenario.math_proficiency_delta || 0).toFixed(1)} percentage points
- ELA proficiency: ${Number(scenario.ela_proficiency_delta) > 0 ? '+' : ''}${Number(scenario.ela_proficiency_delta || 0).toFixed(1)} percentage points
- Graduation rate: ${Number(scenario.grad_rate_delta) > 0 ? '+' : ''}${Number(scenario.grad_rate_delta || 0).toFixed(1)} percentage points
- Chronic absenteeism: ${Number(scenario.absenteeism_delta) > 0 ? '+' : ''}${Number(scenario.absenteeism_delta || 0).toFixed(1)} percentage points
- New counselor ratio: 1:${Math.round(Number(scenario.counselor_ratio_new || 0))}
